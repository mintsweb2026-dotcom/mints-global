...
export function NewsletterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send(
        process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID || 'service_8g6q7x6',
        process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID || 'template_v00c14b',
        {
          to_email: 'info@mintsglobal.ae',
          from_name: email,
          message: 'New newsletter subscription request',
          reply_to: email,
        },
        process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_KEY || 'N_fXzNf-7PjQy9r-q'
      );
