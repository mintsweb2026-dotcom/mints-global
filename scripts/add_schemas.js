...
for (const page of pages) {
  ...
  if (!content.includes('JsonLd')) {
    content = content.replace("import { SEO", "import { JsonLd } from '../components/JsonLd';\nimport { buildServiceSchema, buildFaqSchema, buildBreadcrumbSchema } from '../lib/schema-helpers';\nimport { Breadcrumbs } from '../components/Breadcrumbs';\nimport { SEO");
  }

  // Create schema strings
  const schemaStr = `\n  const serviceSchema = buildServiceSchema({\n    name: "${data.name}",\n    description: "${data.description}",\n    url: "${data.url}",\n    serviceType: "${data.category}"\n  });\n\n  const breadcrumbSchema = buildBreadcrumbSchema([\n    { name: "Home", url: "/" },\n    { name: "${data.category}", url: "${data.categoryUrl}" },\n    { name: "${data.name}", url: "${data.url}" }\n  ]);\n`;
  
  // Add schemas inside function
  if (!content.includes('serviceSchema = buildServiceSchema')) {
    const fnRegex = new RegExp(`export function ${page.replace('.tsx', '')}\\(\\) {`);
    content = content.replace(fnRegex, `export function ${page.replace('.tsx', '')}() {` + schemaStr);
  }

  // Add Breadcrumbs UI
  if (!content.includes('<Breadcrumbs')) {
    // Find first section / div that returns after <SEO ... />
    // Alternatively, let's just insert it after the header (which is usually pt-32)
    // Looking for className="bg-brand-black min-h-screen text-white pt-32 xl:pt-40"
    content = content.replace(/(<div[^>]*pt-32[^>]*>)/, `$1\n      <div className="max-w-7xl mx-auto px-6 lg:px-8 mb-8">\n        <Breadcrumbs items={[\n          { label: 'Home', href: '/' },\n          { label: '${data.category}', href: '${data.categoryUrl}' },\n          { label: '${data.name}', href: '${data.url}' }\n        ]} />\n      </div>`);
  }

  // Add JsonLd tags output before last closing div
  if (!content.includes('<JsonLd data={serviceSchema} />')) {
      ...
      const relatedSectionStr = `
        {/* Related Services */}
        <section className="py-16 border-t border-white/5 max-w-7xl mx-auto px-6 lg:px-8">
          <h3 className="font-display text-2xl font-black uppercase mb-8">Related Services</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Link to="${related[0].url}" className="text-olive-500 hover:text-olive-400 font-bold">${related[0].name} →</Link>
            <Link to="${related[1].url}" className="text-olive-500 hover:text-olive-400 font-bold">${related[1].name} →</Link>
            <Link to="${related[2] ? related[2].url : related[0].url}" className="text-olive-500 hover:text-olive-400 font-bold">${related[2] ? related[2].name : related[0].name} →</Link>
          </div>
        </section>
      `;
      content = content.replace(/(<CTASection \/>\s*<\/div>)/, `$1\n${relatedSectionStr}\n      <JsonLd data={serviceSchema} />\n      {faqs && <JsonLd data={buildFaqSchema(faqs)} />}\n      <JsonLd data={breadcrumbSchema} />`);
  }

  // If there's no <CTASection />, then look for </div> at the end
  if (!content.includes('Related Services') && !content.includes('<CTASection />')) {
      content = content.replace(/(<\/div>\s*)$/, `      <JsonLd data={serviceSchema} />\n      {typeof faqs !== 'undefined' && <JsonLd data={buildFaqSchema(faqs)} />}\n      <JsonLd data={breadcrumbSchema} />\n$1`);
  }

  fs.writeFileSync(p, content, 'utf8');
  console.log('Updated schemas for', page);
}
